...
impl State {
  pub fn endgame(&self) -> Option<PovScore>
  {
    ...
    if all_pawns == 0 {
      ...
      if total == 1 {
        ...
        if major != 0 {
          ...
          let score =
            if side_to_move_atk { PovScore::inexact_win(distance) }
            else {               PovScore::inexact_loss(distance) };
          return Some(score);
        }
        // KB v K and KN v K
    ...
    if all_pawns == 1 {
      ...
      if distance > 0 {
        let score =
          if side_to_move_atk { PovScore::inexact_win(distance as u16) }
          else {                PovScore::inexact_loss(distance as u16) };
        return Some(score);
      }
      return Some(PovScore::ZERO);
    }
