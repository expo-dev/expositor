...
use crate::rand::{uniform, triangular};
...
impl Network {
  ...
  pub fn perturb_fst_snd(&mut self, n1 : usize)
  {
    ...
    for r in 0..REGIONS {
      ...
      for np in 0..Np { region.w1[SameSide][np][n1] += triangular() as f32 * f1; }
      for np in 0..Np { region.w1[OppoSide][np][n1] += triangular() as f32 * f1; }
    }
    let f2 = ((N1*2) as f32).sqrt().recip();
    for h in 0..HEADS {
      let head = &mut self.hd[h];
      for n2 in 0..N2 {
        head.w2[n2][SideToMove ][n1] += uniform() as f32 * f2;
        head.w2[n2][SideWaiting][n1] += uniform() as f32 * f2;
      }
    }
  }
  ...
  pub fn perturb_thd(&mut self)
  {
    ...
    for h in 0..HEADS {
      let head = &mut self.hd[h];
      for n2 in 0..N2 { head.w3[n2] += uniform() as f32 * f3; }
    }
  }
...
#[derive(PartialEq)]
pub enum Image { FstIndp, FstUnif }

impl Network {

  pub fn save_image(&self, file : &str, opt : Image) -> std::io::Result<()>
  {
    let aspect  = 32; // width of image in neurons
    ...
    for n in 0..N1 {
      for r in 0..REGIONS {
        ...
        for side in 0..2 {
          for x in 0..Np {
            let z = self.rn[r].w1[side][x][n];
            if z > upper { upper = z; }
            if z < lower { lower = z; }
          }
        }
        let s = upper.max(-lower);
        scale[n][r] = s;
        unif_scale = unif_scale.max(s);
      }
    }
            ...
            for _ in 0..upscale {
              for tile_column in 0..wasp {
                ...
                for kind in 0..6 {
                  ...
                  for file in 0..8 {
                    let region = (REGIONS-1) - subtile_row;
                    ...
                    let s = match opt { Image::FstIndp => scale[n][region], _ => unif_scale };
                    ...
                    let w1 =
                      if kind == 5 && (rank == 0 || rank == 7) { 0.0 }
                      else { self.rn[region].w1[side][x][n] / s };

                    let normed = match opt {
                      Image::FstIndp => (w1 + 1.0) / 2.0,
                      _              => (ulg(8.0, w1) + 1.0) * 0.5
                    };
                    debug_assert!(
                      1.0 >= normed && normed >= 0.0,
                      "out of range ({} {} {} {} {})",
                      normed, w1, self.rn[region].w1[side][x][n], s,
                      if opt == Image::FstIndp { "indp" } else { "unif" }
                    );

                    const C : f32 = 31.5;
                    let b1 = self.rn[region].b1[n];
                    let bias = if b1 > 0.0 { (b1/scale[n][region]) * (C*2.0) } else { 0.0 };
                    let r = bias + normed * (255.0 - bias*2.0);
                    let g =        normed *  255.0            ;
                    let b =    C + normed * (255.0 -    C*2.0);
                    let r = r.round() as u8;
                    let g = g.round() as u8;
                    let b = b.round() as u8;
...
/*
  pub fn stat(&self)
  {
    ...
    println!("w1"); stat_slice(unsafe { transmute::<_,&[f32; Np*B1]>(&self.w1) });
    println!("b1"); stat_slice(&self.b1);
    for h in 0..HEADS {
      let head = &self.hd[h];
      println!("h{h} w2"); stat_slice(unsafe { transmute::<_,&[f32; N1*N2]>(&head.w2) });
      println!("h{h} b2"); stat_slice(&head.b2);
      println!("h{h} w3"); stat_slice(&head.w3);
    }

    let mut fst_lo   = [0.0; B1];
    let mut fst_hi   = [0.0; B1];
    let mut fst_lo_b = [0.0; B1];
    let mut fst_hi_b = [0.0; B1];

    for n in 0..B1 {
      let mut inp = [0.0; Np];
      for x in 0..Np { inp[x] = self.w1[x][n]; }
      ...
      let lo : f32 = inp[  0  ..16].iter().sum::<f32>() * 2.0;
      let hi : f32 = inp[Np-16..Np].iter().sum::<f32>() * 2.0;
      fst_lo  [n] = lo;
      fst_hi  [n] = hi;
      fst_lo_b[n] = lo + self.b1[n];
      fst_hi_b[n] = hi + self.b1[n];
    }
...
fn lg(k : f32, x : f32) -> f32
{
  return (1.0 + (-k * x).exp2()).recip() - 0.5;
}

fn ulg(k : f32, x : f32) -> f32
{
  return lg(k, x) / lg(k, 1.0);
}
...
/* fn stat_slice(xs : &[f32])
{
  ...
  let max = xs[len-1];
  println!("  {min:+10.6} {p05:+10.6} {med:+10.6} {p95:+10.6} {max:+10.6}");
