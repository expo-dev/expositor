...
//
// If we imagine a and b as two halves, high and low, viz. (a_hi << 32) + a_lo
//   and (b_hi << 32) + b_lo, when multiplied we have
//
//   (a_hi × b_hi) << 64  +  (a_hi × b_lo + a_lo × b_hi) << 32  +  (a_lo + b_lo)
//
// Note that the sum a_lo + b_lo is only 33 bits wide, so after taking the
//   high word (equivalently, right shifting the product by 64) this disappears
//   entirely (except perhaps for propagated carries). In practice, b is about
//   2²³ to 2³³, so b_hi is usually zero, and then a_lo × h_hi is zero.
...
pub fn table_update(key : u64, entry : TableEntry)
{
  ...
  for x in 0..3 {
    ...
    // The argument can overwrite an extent entry if
    // • its generation is newer, or
