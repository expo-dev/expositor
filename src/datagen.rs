...
use crate::piece::Piece::{WhitePawn, BlackPawn};
use crate::global::*;
...
// Quite happily, u/a rapidly converges to a limit as k increases!
// Taking a look at the graph [https://www.desmos.com/calculator/2ixjkpctrh] we can see that
...
) -> std::io::Result<usize>
{
  ...
  loop {
    ...
    if params.include_end {
      ...
      let pawns = (state.boards[WhitePawn] | state.boards[BlackPawn]).count_ones();
      allow = men <= 8 || (men - pawns) <= 4;
    }
    if (allow || score.unsigned_abs() <= params.max_score)
  ...
  for mut leaf in leaves {
    ...
    let mut disallowable = true;
    if params.include_end {
      let men = (leaf.sides[White] | leaf.sides[Black]).count_ones();
      let pawns = (leaf.boards[WhitePawn] | leaf.boards[BlackPawn]).count_ones();
      disallowable = men > 8 && (men - pawns) > 4;
    }
    if disallowable && score.unsigned_abs() > params.max_score { continue; }
...
pub fn sample_syzygy(path : &str, seed : u64) -> std::io::Result<()>
{
  use crate::movegen::Selectivity::Everything;
  ...
  while num_draw + num_mate < 50_000_000 {
    // 5-man
    //   KBPvK  KBvKP  KBPvKP
    //   50%    12.5%  37.5%
    // 6-man
    ...
    for _ in 0..2 {
      ...
      state.generate_legal_moves(Everything, &mut early_moves, &mut late_moves);
      if early_moves.len() > 0 { continue; }
...
) -> (PovScore, Move)
{
  ...
  for step in 1..((MAX_DEPTH+1) as u8) {
    ...
    while searching {
      let alpha = if alpha_failures >= 2 { PovScore::MIN } else { score - alpha_width };
      let  beta = if  beta_failures >= 2 { PovScore::MAX } else { score +  beta_width };
      ...
      if alpha >= tentative { alpha_failures += 1; alpha_width = alpha_width * 2; }
      if tentative >= beta  {  beta_failures += 1;  beta_width =  beta_width * 2; }
    }
    score = tentative;
