...
//
// The range for inevitable inexact mates is somewhat small (10 pawn as opposed to, say,
//   100 pawn) because we want e.g. to use the same widths for aspiration windows and have
//   them be sensible.

const  TERMINAL_WIN : i16 = 250_00; // 100 − 250  winning, exact (proven), terminal
const TABLEBASE_WIN : i16 = 100_00; //  90 − 100  winning, exact (proven)
const   INEXACT_WIN : i16 =  90_00; //  80 −  90  winning
const WIN_THRESHOLD : i16 =  80_00;

const  TERMINAL_LOSS : i16 =  -TERMINAL_WIN;
const TABLEBASE_LOSS : i16 = -TABLEBASE_WIN;
const   INEXACT_LOSS : i16 =   -INEXACT_WIN;
...
impl PovScore {
  ...
  pub const LOST : Self = Self { inner: TERMINAL_LOSS };
  ...
  pub const fn is_invalid(&self) -> bool { return self.abs() > TERMINAL_WIN; }
  ...
  pub const fn is_checkmate(&self) -> bool { return self.inner == TERMINAL_LOSS; }

  pub const fn is_terminal(&self) -> bool { return self.inner.abs() > TABLEBASE_WIN; }
  pub const fn is_exact   (&self) -> bool { return self.inner.abs() > INEXACT_WIN  ; }
  pub const fn is_winloss (&self) -> bool { return self.inner.abs() > WIN_THRESHOLD; }

  pub const fn is_terminal_win (&self) -> bool { return self.inner > TABLEBASE_WIN ; }
  pub const fn is_terminal_loss(&self) -> bool { return self.inner < TABLEBASE_LOSS; }
  pub const fn is_exact_win    (&self) -> bool { return self.inner > INEXACT_WIN   ; }
  pub const fn is_exact_loss   (&self) -> bool { return self.inner < INEXACT_LOSS  ; }
  ...
  pub const fn  terminal_win (distance :  u8) -> Self { return Self { inner:  TERMINAL_WIN  - distance as i16 }; }
  pub const fn  terminal_loss(distance :  u8) -> Self { return Self { inner:  TERMINAL_LOSS + distance as i16 }; }
  ...
  pub const fn   inexact_win (distance : u16) -> Self { return Self { inner:   INEXACT_WIN  - distance as i16 }; }
  pub const fn   inexact_loss(distance : u16) -> Self { return Self { inner:   INEXACT_LOSS + distance as i16 }; }

  pub const fn checkmate_moves(&self) -> i16
  {
    return if self.inner >= 0
        {   (TERMINAL_WIN+1 - self.inner) / 2  }
      else
        { -((TERMINAL_WIN+1 + self.inner) / 2) };
  }
...
impl CoScore {
  ...
  pub const fn is_checkmate(&self) -> bool { return self.inner.abs() == TERMINAL_WIN; }
...
) -> std::fmt::Result
{
  ...
  if abs > TABLEBASE_WIN {
    let moves = (TERMINAL_WIN+1 - abs) / 2;
    ...
    let x = (std::cmp::max(TERMINAL_WIN - abs, 1) - 1) as f64;
    let x = 1.0 - (x * k).exp();
...
impl std::fmt::Octal for PovScore {
  fn fmt(&self, f : &mut std::fmt::Formatter<'_>) -> std::fmt::Result
  {
    ...
    if self.is_terminal() && self.is_valid() {
