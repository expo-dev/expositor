...
) -> PovScore
{
  ...
  && state.dfz == 0
  {
    ...
    if !alpha.is_terminal_win()
    && !beta.is_terminal_loss()
    && syzygy_enabled()
    && syzygy_support() >= men {
  ...
  if entry.hint_move != 0 && okay_generation {
    ...
    if !hint_move.is_null() {
      let score =
        if      entry.hint_score.is_exact_win()  { entry.hint_score - PovScore::new(height as i16) }
        else if entry.hint_score.is_exact_loss() { entry.hint_score + PovScore::new(height as i16) }
  ...
  if context.nominal > 4 && height > 0 && zerowind && !state.incheck {
    ...
    if depth < 8
      ...
      && upper_estimate >= beta + PovScore::new(RFP_OFFSET + (depth as i16)*RFP_SCALE)
    { return beta; }
  ...
  loop {
    ...
    if emit_hint {
      ...
      if depth < 8
        ...
        && lower_estimate + PovScore::new(FP_OFFSET + (depth as i16)*FP_SCALE) < alpha
      { futile = true; }
    ...
    if alpha >= beta {
      ...
      if depth >= 2 { selector.update_history_scores(context, height); }
      break;
    }
  }
  ...
  if successors == 0 {
    best_score =
      ...
      else if state.incheck { PovScore::terminal_loss(height) }
      else { PovScore::ZERO };
  }
  ...
  let kind =
    ...
    //   distinguish Cut nodes from PV nodes when alpha == beta.
      ...
      (selector.stage != Stage::Done || best_score.is_terminal()) { NodeKind::Cut }
  ...
  if excluded_move.is_null() {
    let table_score =
      if      best_score.is_exact_win()  { best_score + PovScore::new(height as i16) }
      else if best_score.is_exact_loss() { best_score - PovScore::new(height as i16) }
      else                               { best_score };
...
)
{
  ...
  'iterative_deepening: for step in 1..(limits.depth+1) {
    ...
    if isatty(STDERR) {
      ...
      // 1 + 3 = 4
      if time_to_depth >= 0.25 {
        let utilization = table_utilization(generation() as u16);
        if utilization > 999 {
...
)
{
  ...
  unsafe {
    for id in 0..num_threads { GLOB.context[id].reset(); }
  }
