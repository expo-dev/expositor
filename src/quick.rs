...
impl MiniState {
  pub fn to_quick(&self) -> String
  {
    ...
    out[0] = ENCODE[( s        & 63) as usize];
    out[1] = ENCODE[((s >>  6) & 63) as usize];
    out[2] = ENCODE[((s >> 12) & 63) as usize | o << 4];
...
#[inline] fn enc(x : u8) -> u8 { return ENCODE[x as usize]; }
#[inline] fn dec(c : u8) -> u8 { return DECODE[c as usize]; }
