...
impl MiniState {
  ...
  pub fn set_outcome(&mut self, outcome : Outcome)
  {
    let new = match outcome {
      Outcome::Unknown => 0b_1010,
      Outcome::Black   => 0b_0000,
      Outcome::Draw    => 0b_0010,
      Outcome::White   => 0b_0100,
    };
...
impl State {
  ...
  {
    debug_assert!(x < 324,  "starting position must be less than 324");
    //
