...
fn build_3man_subtable(x : usize) {
      ...
      for dks in 0..64 {
        ...
        if legal(subtable[akx][aps][dks][1]) {
          ...
          loop {
            ...
            // Can the king escape check?
            let mut mated = true;
            while dests != 0 {
              let dst = dests.trailing_zeros() as usize;
              if legal(subtable[akx][aps][dst][0]) { mated = false; break; }
              dests &= dests - 1;
            }
            if mated { subtable[akx][aps][dks][1] = 0x80; }
            break;
          }
        }
      }
  ...
  for _ in 0..50 {
    ...
    for akx in 0..32 {
      for aps in 0..64 {
        for dks in 0..64 {
          if !legal(subtable[akx][aps][dks][0]) { continue; }
          ...
          while dests != 0 {
            ...
            let fdst = sq2idx(fdst);
            let entry = subtable[fdst][faps][fdks][1];
          ...
          // Update the entry
          if best < 0x40 { subtable[akx][aps][dks][0] = 0xC0 + best + 1; }
        } // dks
      } // aps
    } // akx
    ...
    for akx in 0..32 {
      for aps in 0..64 {
        for dks in 0..64 {
          if !legal(subtable[akx][aps][dks][1]) { continue; }
          let mut best : u8 = 0;
          let mut dests = king_destinations(dks);
          ...
          if best > 0 { subtable[akx][aps][dks][1] = 0x80 + best + 1; }
        } // dks
      } // aps
    } // akx
  }
...
pub fn probe_3man(state : &State, height : i16) -> i16 {
  let w_king_only = state.sides[W].count_ones() == 1;
  ...
  let mut atk_king = if w_king_only { black_king } else { white_king };
  let mut def_king = if w_king_only { white_king } else { black_king };
  ...
  let side_to_move_atk = (w_king_only as usize) ^ (state.turn as usize);
  ...
  let entry = subtable[atk_king][atk_piece][def_king][side_to_move_atk];
...
pub fn probe_3man_line(state : &mut State) -> (i16, Vec<Move>)
{
