  ...
  loop {
    ...
    match cmd {
      ...
      "uci" => {
        ...
        println!("option name Hash type spin default {} min 2 max 262144", CACHE_SIZE_DEFAULT >> 20);
...
*/
      ...
      "tbdata" => {
        ...
        crate::datagen::sample_syzygy(path, seed)?;
      }
      ...
      "view" => {
        ...
        for mini in QuickReader::open(path)? {
          ...
          show(&state, White);
          eprintln!("{} antiphase {}", mini.antiphase(), state.to_fen());
          eprintln!("{:#} static", state.game_eval().from(state.turn));
          ...
          start_search(&state, &history, limits, 1);
...
*/
     ...
     // while default_sz < 16384 { default_sz *= 2; }
      ...
      "color" => {
        if !isatty(STDERR) { continue; }
        let scores = [
          (PovScore::terminal_win( 0).from(Black), PovScore::terminal_loss( 0)),
          (PovScore::terminal_win( 1).from(Black), PovScore::terminal_loss( 1)),
          (PovScore::terminal_win( 3).from(Black), PovScore::terminal_loss( 3)),
          (PovScore::terminal_win( 7).from(Black), PovScore::terminal_loss( 7)),
          (PovScore::terminal_win(15).from(Black), PovScore::terminal_loss(15)),
          (PovScore::terminal_win(31).from(Black), PovScore::terminal_loss(31)),
          ...
          (CoScore::new( 2_00)                   , PovScore::new( 2_00)       ),
          (CoScore::new( 4_00)                   , PovScore::new( 4_00)       ),
          (PovScore::terminal_win(31).from(White), PovScore::terminal_win(31) ),
          (PovScore::terminal_win(15).from(White), PovScore::terminal_win(15) ),
          (PovScore::terminal_win( 7).from(White), PovScore::terminal_win( 7) ),
          (PovScore::terminal_win( 3).from(White), PovScore::terminal_win( 3) ),
          (PovScore::terminal_win( 1).from(White), PovScore::terminal_win( 1) ),
          (PovScore::terminal_win( 0).from(White), PovScore::terminal_win( 0) ),
        ];
        eprintln!();
        eprintln!(" Black     PoV");
        eprintln!("   Win    Loss");
...
COPYRIGHT
  Copyright 2024 <helo@expositor.dev>
