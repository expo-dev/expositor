...
impl State {
  pub fn parse(&self, text : &str) -> Result<Move, &'static str>
  {
    ...
    let (early_moves, late_moves) = self.legal_moves(Everything);
...
impl Move {
  ...
  pub fn disambiguate(&self, state : &State) -> DisambiguatedMove
  {
    ...
    let mut width = buffer.len() as u8;
...
impl std::fmt::Display for DisambiguatedMove {
  fn fmt(&self, f : &mut std::fmt::Formatter<'_>) -> std::fmt::Result
  {
    ...
    if let Some(width) = f.width() {
      let width = if width > 255 { 255 } else { width as u8 };
      let rjust = f.align() == Some(std::fmt::Alignment::Right);
      if self.width < width {
        if !rjust { for _ in 0..(width - self.width) { f.write_char(' ')?; } }
